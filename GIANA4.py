...
# 迭代“blosum62”字典的键，提取氨基酸对，并对相应的值执行一些转换
blosum62n={}
...
# 该字典bl62表示一个替换矩阵，其中每个氨基酸在替换其他每个氨基酸时都与替换分数列表相关联。
# 分数表明氨基酸之间的相似性或不相似性程度。
# 字典的键是单字母氨基酸代码。
# 与每个键关联的值是表示替换分数的列表。列表中分数的位置对应于被替换的氨基酸的索引。
# bl62['A']表示用丙氨酸 ('A') 替换其他每个氨基酸时的替换分数。
# 第一个分数 (-1) 对应于用丙氨酸替换丙氨酸，第二个分数 (4) 对应于用丙氨酸替换精氨酸
...
# 根据字典的值创建一个 NumPy 数组bl62。它首先从字典中提取值（列表）list(bl62.values())
# 然后使用列表理解和将每个列表转换为 NumPy 数组np.array(x)
# 最后，它创建一个包含这些数组的 2D NumPy 数组。
bl62c=np.array([np.array(x) for x in list(bl62.values())])
#  4 中减去 NumPy 数组的每个元素。bl62c这有效地反转了分数，使更高的分数对应于更相似的氨基酸
bl62c=4-bl62c

# 使用 scikit-learn 的MDS类来执行多维缩放（MDS），以将数组表示的数据嵌入bl62c到低维空间中
# n_components：嵌入空间的维度数。
# n_init：MDS 算法以不同初始化运行的次数。最终的结果将是压力方面的最佳输出
# max_iter：优化算法的最大迭代次数。
# eps：公差宣告收敛。
# dissimilarity：指定输入是预先计算的相异矩阵。
embedding=MDS(n_components=13, n_init=100, max_iter=1000, eps=0.00001, dissimilarity='precomputed')
# 这条线将 MDS 模型拟合到相异矩阵bl62c，并将数据转换到低维空间。它计算嵌入空间中点的坐标
X=embedding.fit_transform(bl62c)

# 通过附加零将每个氨基酸的 MDS 转换坐标扩展到一致的长度
# 创建一个新字典bl62np，其中每个键值对都由氨基酸代码和相应的 NumPy 数组组成。
# 该数组是通过将相异矩阵的 MDS 变换的坐标与零连接起来形成的，以确保长度一致。
bl62np={}
# 创建一个vkk包含原始字典中的键（氨基酸代码）的列表bl62。
vkk=list(bl62.keys())
# 此循环在 20 的范围内迭代，假设有 20 个氨基酸代码bl62（从列表的长度推断vkk）
for ii in range(20):
    # 将当前氨基酸代码分配给变量kk。
    kk=vkk[ii]
    # 通过 MDS 转换 ( )的坐标与包含 17 个零的列表的串联创建一个 NumPy 数组。
    # 然后将得到的数组存储在bl62np以氨基酸代码kk为键的字典中。
    bl62np[kk]=np.array(list(X[ii,])+[0]*17)

# 初始化一个空字典AAencodingDict，其中键将是氨基酸字母，值将是其相应的独热编码向量
AAencodingDict={}
for ii in range(len(AAstringList)):
    # 将当前氨基酸字母分配给变量aa。
    aa=AAstringList[ii]
    # 创建一个列表，CODE表示当前氨基酸的 one-hot 编码向量。
    # 将索引处的元素设置ii为 1（表示氨基酸在字母表中的位置），并用零填充列表的其余部分。
    CODE=[0]*(ii)+[1]+[0]*(20-ii)
    # 将 one-hot 编码向量的 NumPy 数组版本分配给字典中的CODE氨基酸字母。
    AAencodingDict[aa]=np.array(CODE)

Ndim=16  ## optimized for isometric embedding 针对等距嵌入进行了优化
n0=Ndim*6
#M0=np.concatenate((np.concatenate((ZERO,M1),axis=1),np.concatenate((M1, ZERO),axis=1)))
# 创建一个用 0 填充、维度为 的2D NumPy 数组Ndim。该变量将是一个大小为ZERO的方阵
ZERO=np.zeros((Ndim,Ndim))
# 使用Ndim来创建大小为单位的矩阵。单位矩阵的对角线上有 1，其他地方有 0。该变量将是一个大小为 的单位矩阵
II=np.eye(Ndim)
# 沿水平轴连接三个矩阵以形成矩阵M0。矩阵的M0维度3*Ndim为3*Ndim
...
## Construct 6-th order cyclic group 构造六阶循环群
# 创建一个用 0 填充、维度为 的2D NumPy 数组Ndim*3
ZERO45=np.zeros((Ndim*3,Ndim*3))
# 沿水平轴和垂直轴连接四个矩阵以创建矩阵M6
...
# s使用矩阵M、附加参数n0和来对给定序列执行编码bl62np
# 该函数本质上使用给定矩阵M和来自 的附加编码向量将序列编码为数值向量bl62np
def EncodingCDR3(s, M, n0):
    # 将输入字符串转换s为字符列表
    sL=list(s)
    # 初始化一个用零填充的numpy数组x，长度为n0.
    x=np.array([0]*n0)
...
def BuildLengthDict(seqs, sIDs, vGene=[], INFO=[]):
    ...
    # LengthD、SeqD、VgeneD、InfoD：
    # 用于存储按长度、V 基因（如果提供）和附加信息（如果提供）分组的序列的字典
    LengthD={}
    SeqD={}
    VgeneD={}
    InfoD={}
    ...
    # 包含非氨基酸字母的序列的计数器
    cNAs=0
    # 检查序列是否包含任何非氨基酸字母。如果是，则跳过该序列，并打印一条警告消息。
    for ii in range(len(seqs)):
        ...
        if L not in LengthD:
            ...
            if len(INFO)>0:
                InfoD[L].append(info)
    # 如果遇到任何带有非氨基酸字母的序列，则会打印一条警告消息，其中包含此类序列的计数。
    # 然后该函数返回包含按长度、V 基因和附加信息分组的序列的字典
    if cNAs>0:
        print("Warning: Skipped %d sequences with non AA letter!" %(cNAs))
    return LengthD, VgeneD, InfoD, SeqD

# 将具有相同长度的序列、V 基因和序列折叠成唯一的代表，并将它们存储在字典中
def CollapseUnique(LD, VD, ID, SD):
    kks=LD.keys()
    # LD、VD、ID、SD：包含分别按长度、V 基因、序列 ID 和序列分组的序列的字典。
    # LDu, VDu, IDu, SDu：折叠后存储序列的唯一代表的字典。
    LDu={}
    VDu={}
    IDu={}
    SDu={}
    # 检索序列、V 基因、序列 ID 和与当前长度对应的序列的列表
    for kk in kks:
        vvL=list(LD[kk])
        if len(VD)>0:
            vvV=list(VD[kk])
        ...
        # 将序列、V基因和序列ID组合成元组，并根据序列和V基因进行排序
        vvI=list(ID[kk])
        vvS=list(SD[kk])
...
class CDR3:
    def __init__(self, s, sID, KS, st, ed):
        ...
        # 应包含在 k 聚合中的第一个氨基酸（从 0 开始）的索引。
        ## ed: the last L-ed amino acids will be skipped
        # 在 k 聚合过程中应跳过的序列末端的氨基酸数量。
        self.s=s #存储输入的CDR3序列
        # 存储CDR3序列的唯一标识符
        self.ID=sID
        # 存储CDR3序列的长度。
        L=len(s)
        # 考虑开始和结束索引，存储从 CDR3 序列生成的 k-mers。
        self.L=L
        sub_s=s[st: (L-ed)]
        Ls=len(sub_s)
...
# 构建 k-mer 集，以便基于允许不匹配的 k-mer 索引进行快速读取搜索。
class KmerSet:
    ## Kmer set for fast read searching based on mismatch-allowed Kmer index
    # 表示用于基于允许不匹配的 k-mer 索引进行有效读取搜索的 k-mer 集。
    def __init__(self, Seqs, sIDs, KS, st, ed):
        ...
        # Seqs : CDR3 序列列表。
        # sIDs：与每个 CDR3 序列对应的唯一标识符列表。
        # KS：k 聚体的大小。
        # st：k-merization 的起始索引
        # ed：k-merization 的结束索引
        if len(Seqs) != len(sIDs):
            raise "Sequence and ID lists have different length. Please check input."
        # 初始化空字典来存储 k-mers 和相应的序列 ID。
        KmerDict={}
        # 使用该类将每个 CDR3 序列解析为 k-mers CDR3
        N=len(Seqs)
        self.N=N
        # 构建一个字典，其中键是 k-mer，值是包含该 k-mer 的序列 ID 列表。
        CDR3Dict={}
        # 存储 k-mer 字典 ( KD)、k-mer 大小 ( KS)、CDR3 字典 ( CD) 和 CDR3 长度列表 ( LL)。
        LLs=[]
        for ii in range(0,N):
            s=Seqs[ii]
            sID=sIDs[ii]
        ...
        # KD：字典，其中键是 k-mer，值是包含该 k-mer 的序列 ID 列表
        self.KD=KmerDict
        # KS：k 聚体的大小
        self.KS=KS
        # CD：字典，其中键是序列 ID，值是从相应 CDR3 序列生成的 k-mers 列表。
        self.CD=CDR3Dict
        # LL：包含所有 CDR3 序列长度的列表
        self.LL=LLs
    def FindKmerNeighbor(self,kk):
        ...
        for jj in range(KS):
            ...
            # 对于每个位置，通过用所有其他氨基酸替换当前氨基酸来生成所有可能的 k 聚体。
            kkn=list(zip(kk_pre,AAstringList,kk_suf))
            # 将生成的k-mers添加到邻居列表中KS_n1。
    ...
    def FindKmerNeighbor2(self,kk):
        ...
        # 初始化一个空列表KS_n1来存储邻居。
        KS_n1=[]
        # 迭代 k 聚体中的每个位置，由索引jj和表示ii
        for jj in range(KS):
            for ii in range(KS):
                # 通过考虑所有位置组合，生成具有两个不匹配的所有可能的 k 聚体jj，ii使得ii大于jj。
                if ii<=jj:
                    continue
                # 将位置处的氨基酸替换jj为ii所有其他氨基酸，生成所有可能的组合。
                kk_pre=[kk[0:jj]]*20
    ...
    def KmerIndex(self):
        ...
        # 获取存储在实例属性中的所有 k-mers ( KKs)的列表。
        # 转换KKs为一组KKs_set以进行更快的成员资格测试。
        # 将所有 k-mers 连接成一个Skk由下划线分隔的字符串。该字符串将用于模式匹配
        KKs=list(self.KD.keys())
        KS=self.KS
        KKs_set=set(KKs)
        Skk='_'.join(KKs)
        KI_Dict={}
        # 迭代 k-merkk列表中的每个 k-mer ( KKs)。
        for kk in KKs:
##            kk_neighbor=[]
##            for jj in range(KS):
##                kk_pre=kk[0:jj]
...
##                kk_neighbor+=mm
            # 对于每个 k-mer kk，通过调用该方法查找具有单字符不匹配的最近邻居FindKmerNeighbor，
            # 该方法返回一组潜在邻居
            # 将潜在邻居集与所有 k 聚体集相交KKs_set以过滤掉有效邻居
            KS_n=set(self.FindKmerNeighbor(kk))
            # 将有效邻居列表存储在字典中KI_Dict作为kk键。
    ...
    def updateKD(self, KI):
        ...
        # KI：一种字典，其中键是 k-mers，值是具有一个字符不匹配的最近邻 k-mers 列表。
        KD=self.KD
        # KDnew：基于 k-mer 索引更新的新 k-mer 字典
        KDnew={}
        for kk in KD:
            # KD从实例中检索当前的 k-mer 字典KmerSet
            kkm=KI[kk]
