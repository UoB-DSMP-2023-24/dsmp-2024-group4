...
def encode_cdr3(seq,bio_matrix,maxlength):
...
def encode_cdr3_list(seq_list,bio_matrix_name,maxlength):
    bio_matrix = substitution_matrices.load(bio_matrix_name)
    amino_acids = sorted(set(k[0] for k in bio_matrix.keys()).union(set(k[1] for k in bio_matrix.keys())))

    aa_df = pd.DataFrame(index=amino_acids, columns=amino_acids)
    for (aa1, aa2), score in bio_matrix.items():
        aa_df.at[aa1, aa2] = score
        aa_df.at[aa2, aa1] = score  # Ensure symmetry
    standard_protein_letters = "IRQCYMLVAFNESHKWGDTP"
    aa_df = aa_df.loc[list(standard_protein_letters), list(standard_protein_letters)]
    ...
    print('Encoding cdr3 sequences...')
    encoded_seqs = []
    for seq in tqdm.tqdm(seq_list):
        encoded_seqs.append(encode_cdr3(seq, aa_df, maxlength))
